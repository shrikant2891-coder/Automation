...
------------------------
Dynamically builds the Uploader sheet from Extract2 + GL Master.

Fixes the VLOOKUP/SUMIFS limitation: new states and product categories
appearing in Extract2 are discovered automatically and posted — no hardcoded
...
def row_gst_rate(row, base_col_value: float, tax: float = 0.0) -> int:
    ...
    for key in ("cgst_rate", "CGST_RATE"):
        if row.get(key) not in (None, "", 0, 0.0, "0", "0.0"):
            try:
                ...
                pass
    return snap_gst_rate(tax, base_col_value)


def classify_tax_for_row(row, tax: float, base: float):
    rate = row_gst_rate(row, base, tax)
    return classify_tax(
        tax,
        base,
        row.get("state_code_from"),
...
def gen_sales(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        for gl, st_a, tax_amt in classify_tax_for_row(row, tax, rev):
...
def gen_sales_return(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        for gl, st_a, tax_amt in classify_tax_for_row(row, rtax, sret):
            tax_agg[(gl, st_a)] += tax_amt
...
def gen_shipping(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        for gl, st_a, tax_amt in classify_tax_for_row(row, stax, ship):
            if tax_amt >= 0:
...
def gen_price_drop(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        debtor[sc] += pd + pdt
        rev_agg[(rg, st)] += pd
        for gl, st_a, tax_amt in classify_tax_for_row(row, pdt, pd):
            tax_agg[(gl, st_a)] += tax_amt
...
def gen_buyer_fee(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        st_to = row.get("state_code_to")
        st = alias_state(st_from)
        fee = amt_any(row, "Buyer Fee", "Buyer Fees")
        tax = amt_any(row, "Buyer Tax", "BuyerTax")
        debtor[sc] += fee + tax
        if fee >= 0:
            rev_cr[(rev_gl, st)] += fee
        else:
            rev_dr[(rev_gl, st)] += abs(fee)
        for gl, st_a, tax_amt in classify_tax_for_row(row, tax, fee):
            if tax_amt >= 0:
                tax_cr[(gl, st_a)] += tax_amt
            else:
                tax_dr[(gl, st_a)] += abs(tax_amt)
...
def gen_prexo_bumpup(b: UploaderBuilder, rows, maps, voucher_no, date, narration, function):
    ...
    for row in rows:
        ...
        st_to = row.get("state_code_to")
        st = alias_state(st_from)
        rev = amt_any(row, "Prexo Bumpup", "PREXOBUMPUP")
        tax = amt_any(row, "Prexo BumpTAX", "PREXOBUMP_TAX")
        debtor += rev + tax
...
def write_config(ws, vouchers, month_label, date):
    ...
    ws["A19"] = (
        "1) Paste latest extract into the Extract2 sheet (keep header row; includes igst_rate / cgst_rate / sgst_utgst_rate). "
...
def write_instructions(ws):
    ...
    lines = [
        ...
        ("", False),
        ("Problem solved", True),
        ("Earlier Uploader used hardcoded SUMIFS/VLOOKUP rows per state × category. When Extract2 had a new state or product category, those amounts were missed.", False),
        ("This rebuild discovers every state and category present in Extract2 and posts them using GL Master lookups.", False),
        ("", False),
        ...
        ("Tax GLs               = IGST/CGST/SGST by igst_rate / cgst_rate from Extract2", False),
        ("State aliases         = IN-GJ posted as IN-GR; IN-OR posted as IN-OS (Uploader convention)", False),
        ("", False),
        ("How to refresh after a new Extract2", True),
...
def read_config_month(wb):
    if "Config" in wb.sheetnames:
        ws = wb["Config"]
        label = ws["B2"].value
        date_val = ws["B3"].value
        if label:
            month_label = str(label).strip()
            if isinstance(date_val, datetime):
                return month_label, date_val
            return month_label, parse_month_end(month_label)
    if "Extract" in wb.sheetnames:
        legacy_month = wb["Extract"].cell(2, 1).value
        if legacy_month:
            month_label = str(legacy_month).strip()
            return month_label, parse_month_end(month_label)
    return None, None
...
def build(input_path: Path, output_path: Path):
    ...
    extract_sheet = EXTRACT_SHEET if EXTRACT_SHEET in wb.sheetnames else "Extract"
    if extract_sheet not in wb.sheetnames or "GL Master" not in wb.sheetnames:
        raise SystemExit(f"Workbook must contain {EXTRACT_SHEET} (or Extract) and GL Master sheets")

    headers, idx, extract_rows = read_extract(wb[extract_sheet])
    if not extract_rows:
        raise SystemExit(f"{extract_sheet} sheet has no data rows")

    month_label, book_date = read_config_month(wb)
    if not month_label:
    ...
    print(f"Source: {extract_sheet} | Month: {month_label} | Date: {book_date.date()} | Reports: {present_reports}")
    print(f"Extract rows: {len(extract_rows)} | Sales GLs: {len(maps['rev'])} | Return GLs: {len(maps['ret'])}")
    if has_rate_cols(headers):
        print("Tax rates: using igst_rate / cgst_rate columns from extract")
    ...
    order = ["Instructions", "Config", "Uploader", "Extract2", "Extract", "GL Master", "Control", "Unmapped"]
...
def main():
    ap = argparse.ArgumentParser(description="Build RetailX Uploader from Extract2")
    ap.add_argument("--input", default="RetailX.xlsm")
    ap.add_argument("--output", default="RetailX.xlsm")
    args = ap.parse_args()
