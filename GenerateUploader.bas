...
'
' Sheets required: Extract2, GL Master, Uploader
' Optional:       Config (month label / booking date / voucher numbers)
'===============================================================================

Private Const SHEET_EXTRACT As String = "Extract2"

Private Const COL_REPORT As Long = 1
Private Const COL_SCTYPE As Long = 2
Private Const COL_ST_FROM As Long = 4
Private Const COL_ST_TO As Long = 5
Private Const COL_CAT As Long = 6
Private Const COL_IGST_RATE As Long = 7
Private Const COL_CGST_RATE As Long = 8
Private Const COL_SGST_RATE As Long = 9
Private Const COL_SALES_REV As Long = 12
Private Const COL_SALES_DISC As Long = 13
Private Const COL_SALES_TAX As Long = 14
Private Const COL_PBO_REV As Long = 15
Private Const COL_PBO_TAX As Long = 16
Private Const COL_PICKUP_REV As Long = 18
Private Const COL_PICKUP_TAX As Long = 19
Private Const COL_SALES_RET As Long = 20
Private Const COL_RET_DISC As Long = 21
Private Const COL_RET_TAX As Long = 22
Private Const COL_SHIP As Long = 23
Private Const COL_SHIP_TAX As Long = 24
Private Const COL_PD As Long = 25
Private Const COL_PD_TAX As Long = 26
Private Const COL_BUYER As Long = 27
...
Public Sub GenerateUploader()
    ...
    Set wsEx = ThisWorkbook.Worksheets(SHEET_EXTRACT)
    ...
    If lastEx < 2 Then Err.Raise vbObjectError + 1, , SHEET_EXTRACT & " has no data"
...
Private Sub BuildSales(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        End If
        AccTax taxAgg, wsEx, r, tax, rev, stFrom, stTo
NextSales:
    Next r
...
Private Sub BuildSalesReturn(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        End If
        AccTax taxAgg, wsEx, r, rtax, sret, stFrom, stTo
NextRet:
    Next r
...
Private Sub BuildShipping(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        AccTax tmpTax, wsEx, r, stax, ship, stFrom, stTo
...
Private Sub BuildPBO(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        End If
        Set tmpTax = CreateObject("Scripting.Dictionary")
        AccTax tmpTax, wsEx, r, tax, rev, stFrom, stTo
        For Each k In tmpTax.Keys
            amt = CDbl(tmpTax(k))
...
Private Sub BuildPriceDrop(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        AccTax taxAgg, wsEx, r, pdt, pd, stFrom, stTo
NextPD:
    Next r
...
Private Sub BuildBuyerFee(wsUp As Worksheet, wsEx As Worksheet, lastEx As Long, _
    ...
    For r = 2 To lastEx
        ...
        End If
        Set tmpTax = CreateObject("Scripting.Dictionary")
        AccTax tmpTax, wsEx, r, tax, fee, stFrom, stTo
        For Each k In tmpTax.Keys
            amt = CDbl(tmpTax(k))
            If amt >= 0 Then
                Acc taxCr, CStr(k), amt
...
Private Sub AccTax(agg As Object, wsEx As Worksheet, ByVal r As Long, ByVal tax As Double, _
                   ByVal baseAmt As Double, ByVal stFrom As String, ByVal stTo As String)
    Dim rate As Long, st As String
    If Abs(tax) < 0.0000001 Then Exit Sub
    rate = RowGstRate(wsEx, r, tax, baseAmt)
...
Private Function RowGstRate(wsEx As Worksheet, ByVal r As Long, ByVal tax As Double, _
                            ByVal baseAmt As Double) As Long
    Dim igst As Double, cgst As Double
    igst = Nz(wsEx.Cells(r, COL_IGST_RATE).Value)
    cgst = Nz(wsEx.Cells(r, COL_CGST_RATE).Value)
    If igst > 0# Then
        RowGstRate = NormalizeRate(igst)
        Exit Function
    End If
    If cgst > 0# Then
        RowGstRate = NormalizeRate(cgst * 2#)
        Exit Function
    End If
    RowGstRate = SnapRate(tax, baseAmt)
End Function

Private Function NormalizeRate(ByVal pct As Double) As Long
    If pct <= 6# Then
        NormalizeRate = 5
    ElseIf pct <= 15# Then
        NormalizeRate = 12
...
Private Sub LoadConfig(ByRef monthLabel As String, ByRef bookDate As Date, _
    ...
    If Not wsCfg Is Nothing Then
        ...
        End If
    End If
    If Len(Trim$(monthLabel)) = 0 Then monthLabel = "Unknown"
